...
from transformers.tokenization_bert import BertJapaneseTokenizer
...
SENTNCEPIECE_MODEL_NAME = 'wiki-ja.model'
SENTENCEPIECE_VOCAB_NAME = 'wiki-ja.vocab'
ENCODER_DIRNAME = 'bert_encoder'
DECODER_DIRNAME = 'bert_decoder'
SUMMERY_OUTPUT_DIRNAME = 'generated_summaries'
...
def main():
    ...
    parser.add_argument(
        "--pretrained_sentencepiece_model_ja",
        default=None,
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The path to a pre-trained BERT model.",
    )
    parser.add_argument(
        "--pretrained_encoder_decoder_model",
        default=None,
        type=str,
        help="The path to a pre-trained encoder_decoder model.",
    )
    parser.add_argument(
    ...
    if args.pretrained_bert_model and args.pretrained_encoder_decoder_model:
        raise ValueError("It is not allowed to specify both pretrained_bert_model and pretrained_encoder_decoder_model")
    ...
    if args.pretrained_encoder_decoder_model:
        ...
        print(f'encoder_config: {config.encoder_config}')
        print(f'decoder_config: {config.decoder_config}')
        encoder_checkpoint = os.path.join(args.pretrained_encoder_decoder_model, ENCODER_DIRNAME)
        decoder_checkpoint = os.path.join(args.pretrained_encoder_decoder_model, DECODER_DIRNAME)
        model = PreTrainedEncoderDecoder.from_pretrained(
            encoder_checkpoint, decoder_checkpoint
        )
    elif args.pretrained_bert_model:
        print(f'load pre-trained encoder bert model...')
        encoder_checkpoint = args.pretrained_bert_model
        decoder_model = BertForMaskedLM(config.decoder_config)
        model = Model2Model.from_pretrained(encoder_checkpoint, decoder_model=decoder_model)
    else:
        encoder_model = BertModel(config.encoder_config)
        decoder_model = BertForMaskedLM(config.decoder_config)
        model = Model2Model(encoder=encoder_model, decoder=decoder_model)
    ...
    if args.do_evaluate:
        ...
        for checkpoint in checkpoints:
            encoder_checkpoint = os.path.join(checkpoint, ENCODER_DIRNAME)
            decoder_checkpoint = os.path.join(checkpoint, DECODER_DIRNAME)
            model = PreTrainedEncoderDecoder.from_pretrained(
                encoder_checkpoint, decoder_checkpoint
            )
            model.to(args.device)
            ...
            path_to_generated_summaries = os.path.join(
                args.output_dir, SUMMERY_OUTPUT_DIRNAME
            )
